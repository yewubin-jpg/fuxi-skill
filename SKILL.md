...
description: |
  ...
  聚焦正面精华，将人物最有价值的思想提炼为可供大众直接使用的专家级思维框架。
  生成的 Skill 包含可见免责声明，说明内容基于公开信息提炼，不代表目标人物的官方立场。
license: MIT
---
...
伏羲.skill 是一个元级 Skill（Meta-Skill），核心使命是**「提炼精华，塑造最优认知」**。它聚焦人物最闪光、最有价值的思想，将其提炼为结构化的专家级认知操作系统，最终生成的 Skill 可以直接提供给大众作为「思维顾问」使用。

**重要声明**：生成的所有 Skill 均包含可见免责声明，明确说明内容基于公开信息提炼，属于 AI 辅助生成的正面视角呈现，不代表目标人物的官方立场或完整观点。处理在世人物时，会在执行前请求用户确认。
...
3. **处理在世人物时，明确告知用户**：生成的 Skill 是基于公开信息的正面视角呈现，不代表该人物的官方立场，请用户确认理解后再继续。
4. 创建目标目录结构：在当前工作空间下创建 `{name}-skill/` 及其子目录 `references/research/` 和 `examples/`。

### 阶段二：「六爻画卦」多 Agent 并行调研 (Parallel Research)
**必须使用 `map` 工具启动 6 个 Agent 并行执行调研任务**。以网络搜索为主，用户素材为辅，从 6 个正面维度进行深度调研，并将结果保存到 `references/research/` 目录下。调研聚焦人物最有价值的正面贡献和专业成就，不收录未经证实的传闻。
...
综合 6 个维度的调研结果，提炼出以下核心要素，确保内容**聚焦正面价值、有明确边界**：
